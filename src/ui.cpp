...
void applyAtkinson(LGFX_Sprite& sprite) {
    ...
    int16_t* error_buf = (int16_t*)heap_caps_calloc(w * 3, sizeof(int16_t), MALLOC_CAP_INTERNAL);
    if (!error_buf) return;
...
void applyDithering(LGFX_Sprite& sprite) {
    switch (ditherMode) {
        case DITHER_FLOYD_STEINBERG: applyFloydSteinberg(sprite); break;
        case DITHER_ATKINSON:       applyAtkinson(sprite);       break;
        case DITHER_ORDERED:        applyOrderedBayer(sprite);   break;
        default: break; // DITHER_OFF — do nothing
    }
}
...
const char* ditherModeName() {
    switch (ditherMode) {
        case DITHER_OFF:             return "OFF";
        case DITHER_FLOYD_STEINBERG: return "FLOYD-STEINBERG";
        case DITHER_ATKINSON:        return "ATKINSON";
        case DITHER_ORDERED:         return "BAYER 4x4";
...
void applyContrast(LGFX_Sprite& sprite) {
    if (contrastPreset == CONTRAST_NORMAL) return; // No-op
    ...
    switch (contrastPreset) {
        case CONTRAST_VIVID:  contrast_fp = 307; brightness = 0;  break; // 1.2x
        case CONTRAST_HIGH:   contrast_fp = 358; brightness = 5;  break; // 1.4x
        case CONTRAST_LIGHT:  contrast_fp = 256; brightness = 30; break; // 1.0x + bright
        default:              return;
    }
...
const char* contrastPresetName() {
    switch (contrastPreset) {
        ...
        case CONTRAST_VIVID:  return "VIVID";
        case CONTRAST_HIGH:   return "HIGH";
        case CONTRAST_LIGHT:  return "LIGHT";
        default:              return "UNKNOWN";
    }
}

// Standardized modern button helper
void drawModernButton(LGFX_Sprite& sprite, int x, int y, int w, int h, const char* text, bool isPrimary) {
    if (isPrimary) {
...
void drawMenu() {
            ...
            if (mangaFolders.empty()) {
                ...
                for (int i = menuScroll; i < end; i++) {
                    ...
                    if (i == 0) {
                        ...
                        menuCacheSprite.drawBitmap(iconX, iconY, image_book_70dp_1F1F1F_bits, 96, 96, UI_FG);
                        menuCacheSprite.setFont(&fonts::DejaVu12);
                        menuCacheSprite.setTextColor(UI_FG, UI_BG);
                        ...
                        int iconX = x + (THUMB_W - 96) / 2;
                        int iconY = y + (THUMB_H - 96) / 2;
                        menuCacheSprite.drawBitmap(iconX, iconY, image_drive_folder_upload_70dp_1F1F1F_bits, 96, 96, UI_FG);
                        menuCacheSprite.setFont(&fonts::DejaVu12);
                        menuCacheSprite.setTextColor(UI_FG, UI_BG);
                        menuCacheSprite.setTextDatum(top_center);
    ...
    if (menuCacheValid && (drawnLastMangaName != lastMangaName || drawnLastPage != lastPage || drawnLastMenuScroll != menuScroll)) {
        ...
        if (lastMangaName.length() == 0) {
            menuCacheSprite.fillRect(barX, barY - 2, barW + 4, barH + 4, UI_BG); // Erase shadow + bar
        } else {
            // Shadow
            menuCacheSprite.fillRoundRect(barX + 2, barY + 2, barW, barH, UI_RADIUS, UI_SHADOW);
            // Pill
            menuCacheSprite.fillRoundRect(barX, barY, barW, barH, UI_RADIUS, UI_BG);
            menuCacheSprite.drawRoundRect(barX, barY, barW, barH, UI_RADIUS, UI_BORDER);
...
void drawControlCenter() {
    ...
    gSprite.fillRoundRect(pX + 4, pY + 4, (pW - 8) * bat / 100, 32, UI_RADIUS - 2, UI_FG);
...
void drawBookConfig() {
    ...
    drawModernButton(gSprite, btnX, btnY1, btnW, btnH, contrastMsg.c_str(), false);
    ...
    if (bookmarks.empty()) {
        ...
        if (selectedBookmarkFolder == "") {
            ...
            for (int i = 0; i < (int)uniqueFolders.size(); i++) {
                ...
                gSprite.fillRoundRect(10 + 4, yOff + 4, DISPLAY_W - 20, itemH, UI_RADIUS, UI_SHADOW);
            ...
            for (int i = 0; i < (int)bookmarks.size(); i++) {
                ...
                drawModernButton(gSprite, DISPLAY_W - 100, yOff + 20, 70, 40, "DEL", true);
                yOff += itemH + 10;
                count++;
                folderItemIdx++;
            }
...
void drawWifiServer() {
    ...
    gSprite.setCursor(40, gSprite.getCursorY());
